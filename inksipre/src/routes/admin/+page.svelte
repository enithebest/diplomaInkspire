...
<h1 class="text-3xl font-bold mb-6">🛠️ Adminbereich – Produktverwaltung</h1>

<!-- Neues Produkt -->
<form method="POST" action="?/create" use:enhance class="bg-gray-100 p-4 rounded-lg mb-6">
	...
	<div class="grid grid-cols-2 gap-2">
		...
		<input name="base_price" placeholder="Basispreis (€)" type="number" step="0.01" class="border p-2 rounded" required />
		<textarea name="description" placeholder="Beschreibung" class="border p-2 rounded col-span-2"></textarea>
	</div>
	...
	<div class="border rounded-lg p-4 mb-6 bg-white shadow-sm">
		...
		<div class="mt-4">
			...
			<form method="POST" action="?/addVariant" use:enhance class="flex flex-wrap gap-2">
				...
				<input name="price" type="number" step="0.01" placeholder="Preis (€)" class="border p-1 rounded w-28" />
